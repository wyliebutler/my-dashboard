...
router.post('/', authMiddleware_1.default, (req, res) => {
    ...
    db.get(`SELECT MAX(position) as maxPos FROM tiles WHERE userId = ? AND ${groupFilter}`, groupParams, (err, row) => {
        ...
        db.run('INSERT INTO tiles (userId, name, url, icon, groupId, position) VALUES (?, ?, ?, ?, ?, ?)', [userId, name, url, icon, groupId, newPosition], function (err) {
            if (err)
                return res.status(500).json({ message: 'Error creating tile', error: err.message });
            res.status(201).json({ id: this.lastID, name, url, icon, groupId, position: newPosition });
        });
    });
});
// Update Tile
router.put('/:id(\\d+)', authMiddleware_1.default, (req, res) => {
    if (!req.user)
        return res.sendStatus(401);
    const { id } = req.params;
    const { name, url, icon, groupId } = req.body;
...
// Delete Tile
router.delete('/:id(\\d+)', authMiddleware_1.default, (req, res) => {
    if (!req.user)
        return res.sendStatus(401);
    const { id } = req.params;
    const userId = req.user.id;
    (0, database_1.dbRun)('DELETE FROM tiles WHERE id = ? AND userId = ?', [id, userId])
        .then(() => {
...
// Move Tile
router.put('/move', authMiddleware_1.default, (req, res) => {
    if (!req.user)
        return res.sendStatus(401);
    ...
    db.serialize(() => {
        ...
        db.run(`UPDATE tiles SET position = position + 1 WHERE userId = ? AND ${newGroupFilter} AND position >= ?`, [...newGroupParams, newPosition], (err) => {
            if (err) {
                db.run('ROLLBACK');
                return res.status(500).json({ message: 'Error making space in new group', error: err.message });
            }
            db.run('UPDATE tiles SET groupId = ?, position = ? WHERE id = ? AND userId = ?', [newGroupId, newPosition, tileId, userId], (err) => {
                if (err) {
                    db.run('ROLLBACK');
                ...
                db.run('COMMIT', (err) => {
                    if (err)
                        return res.status(500).json({ message: 'Failed to commit move' });
