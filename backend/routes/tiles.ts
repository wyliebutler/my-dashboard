...
// Simple Mutex for SQLite transactions
let dbLock = Promise.resolve();
const runWithLock = async (fn: () => Promise<void>) => {
    const next = dbLock.then(() => fn().catch(console.error));
    dbLock = next;
    return next;
};
...
router.post('/', authenticateToken, (req: AuthRequest, res: Response) => {
    ...
    if (!name || (!url && type !== 'stats') || (!icon && type !== 'stats')) {
        return res.status(400).json({ message: 'Name, URL, and Icon are required for standard links' });
    }
...
router.delete('/:id(\\d+)', authenticateToken, (req: AuthRequest, res: Response) => {
    ...
    const { id } = req.params;
    const userId = req.user.id;
    dbRun('DELETE FROM tiles WHERE id = ? AND userId = ?', [Number(id), userId])
        .then(() => {
        ...
        return new Promise<void>((resolve) => {
            ...
            db.serialize(() => {
                db.run('BEGIN TRANSACTION', (err) => {
                    if (err) {
                        res.status(500).json({ message: 'Database busy' });
                        return resolve();
                    }
                });
