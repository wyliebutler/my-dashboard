...
app.put('/api/tiles/:id', authenticateToken, (req, res) => {
  const { id } = req.params;
...
app.delete('/api/tiles/:id', authenticateToken, (req, res) => {
  const { id } = req.params;
  const userId = req.user.id;
...
app.put('/api/tiles/order', authenticateToken, (req, res) => {
  ...
  db.serialize(() => {
    ...
    orderedIds.forEach((id, index) => {
      // Note: The params for stmt.run are (position, id, ...rest of params)
      stmt.run(index, id, ...params.slice(1), (err) => { // slice(1) to avoid duplicate userId
